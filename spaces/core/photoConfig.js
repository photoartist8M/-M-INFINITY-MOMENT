...
//  5. ★追加：写真の向き(orientation)によって想定アスペクト比を分け、
//     横長/縦長それぞれで実測に近いサイズ計算を行うようにした
// ======================================================

const TIER_COUNT = 5; // 段数
...
const SLOT_MARGIN = 0.50; // スロット幅に対する余白(小さいほど余白大)

// ★追加：写真の向き(orientation)によって想定アスペクト比を分ける
const ASPECT_BY_ORIENTATION = {
  landscape: 1.5, // 横長写真の実測に近い想定値
  portrait: 0.7,  // 縦長写真の実測に近い想定値
};
function getAssumedAspect(photo) {
  return ASPECT_BY_ORIENTATION[photo.orientation] ?? ASPECT_BY_ORIENTATION.landscape;
}
...
export function buildPhotoConfig(photoData) {
  ...
  tiers.forEach((tierPhotos, tier) => {
    ...
    tierPhotos.forEach((photo, indexInTier) => {
      ...
      // ★変更：固定のMAX_ASPECT_ASSUMEDではなく、写真ごとのorientationに応じた想定アスペクト比を使う
      const maxScaleForWidth = maxAllowedWidth / (4.5 * getAssumedAspect(photo));
